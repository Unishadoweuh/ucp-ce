...
export default function InstanceList() {
    ...
    const load = useCallback(() => {
        ...
        fetchInstances()
            ...
            .catch((err) => setError(err.response?.data?.detail || 'Failed to load instances'))
    ...
    const handleAction = async (node: string, vmid: number, action: string) => {
        try {
            await instanceAction(node, vmid, action);
            setSnack({ open: true, message: `${action} sent to VM ${vmid}` });
    ...
    const handleDelete = async () => {
        ...
        try {
            await deleteInstance(deleteDialog.node, deleteDialog.vmid);
            setSnack({ open: true, message: `VM ${deleteDialog.name} deleted` });
    ...
    const columns: GridColDef[] = [
        ...
        },
        {
            ...
            disableColumnMenu: true,
            renderCell: (params) => {
                const row = params.row as Instance;
                    ...
                    <Box sx={{ display: 'flex', gap: 0.25 }}>
                        ...
                        {isRunning && (
                            <>
                                ...
                                <Tooltip title="Reset">
                                    <IconButton size="small" onClick={() => handleAction(row.node, row.vmid, 'reset')}>
        ...
        <Box>
            ...
            {error && (
                <Alert severity="warning" sx={{ mb: 2 }}>
                    {error} — Configure your Proxmox connection to see instances.
                </Alert>
            )}
            ...
            <Dialog open={!!deleteDialog} onClose={() => setDeleteDialog(null)}>
                ...
                <DialogContent>
                    <DialogContentText>
                        Are you sure you want to delete <strong>{deleteDialog?.name}</strong> (VMID: {deleteDialog?.vmid})?
                        This action cannot be undone. The VM must be stopped first.
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
